...
> **EN** — A pixel-art office overlay for Claude Code: every running session becomes a desk
> robot — coding at its desk, phone-scrolling while waiting for your approval, reading on the
> couch when idle. Transparent always-on-top map with token stats, a chat card per session,
> one-click rehire, and SSH multi-server support. Docs are in Traditional Chinese;
> the code comments too. PRs/issues in English are welcome.
...
| 文件 | 內容 |
|------|------|
| [docs/USAGE.md](docs/USAGE.md) | **使用手冊**：操作、狀態表、看板/對話卡/設定卡、SSH、debug、疑難排解 |
