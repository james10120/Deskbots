...
- **工作看板**：每個 session 的負荷量表（context 佔用）、LV、產出/閱讀 token、回合數；
  點卡片（或點機器人）把該 session 的終端叫到最前，每張卡片還有 `/clear` `/compact` `⎋` 快速指令鈕
- **中英雙語介面**：設定卡一鍵把整個介面在中文／English 間切換（首次依 OS 語系自動判定）
...
app/winfocus.py  ←──（看板卡片聚焦/快速指令：Win32 聚焦 + 鍵盤注入）
```
...
| 文件 | 內容 |
|------|------|
| [docs/USAGE.zh-TW.md](docs/USAGE.zh-TW.md) | **使用手冊**：操作、狀態表、看板/快速指令/設定卡、語言切換、SSH、debug、疑難排解 |
...
2. 完成後遠端 session 以「`專案@機器`」出現；看板卡片有「在 VS Code 開啟」鈕直達該機該資料夾
...
godot/
  ...
  usage_board.gd     工作看板：用量卡片（聚焦＋快速指令）＋人才庫
  settings_window.gd 設定卡；lang.gd 中英介面字串表
