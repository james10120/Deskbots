...
var _upgrade: UpgradeWindow   # 🛠 強化卡（餵養/買物資/升級）
...
var _upgrade_btn: Button       # 右上角「強化」鈕
...
func _make_corner_buttons() -> void:
	...
	var ug := Button.new()
	ug.text = Lang.t("c_upgrade")
	ug.tooltip_text = Lang.t("c_upgrade_tip")
	ug.focus_mode = Control.FOCUS_NONE
	ug.size = Vector2(48, 26)
	ug.position = Vector2(get_window().size.x - 208, 4)
	ug.add_theme_font_size_override("font_size", 13)
	ug.pressed.connect(_toggle_upgrade)
	cl.add_child(ug)
	_upgrade_btn = ug
...
func _toggle_upgrade() -> void:
	if _upgrade.visible:
		...
		_upgrade.open_at(get_window().position + (get_window().size - _upgrade.size) / 2)
...
func _update_hud() -> void:
	if _hud != null:
		_hud.text = Lang.t("hud_econ") % [Util.fmt_tok(int(Economy.funds)), Util.fmt_tok(int(Economy.supplies))]
...
func _relabel_chrome() -> void:
	...
	if _upgrade_btn != null:
		_upgrade_btn.text = Lang.t("c_upgrade")
		_upgrade_btn.tooltip_text = Lang.t("c_upgrade_tip")
	if _upgrade != null:
		_upgrade.title = Lang.t("up_title")
	if _hook_hint != null:
		_hook_hint.text = Lang.t("hook_hint")
